...
num_national_county_diff <- all_totals %>%
  mutate(county_diff = cases - county_total) %>%
  filter(county_diff != 0) %>%
  nrow()
...
-------------------------------------------------
  ...
  # What proportion of counties has 0 deaths in each state, save the answer as 'counties_zero_death'?

  #  Write code (at least 2-3 lines) that will answer your question
counties_zero_death <- counties %>%
  group_by(state) %>%
  filter(date == max(date)) %>%
  filter(deaths == 0) %>%
  summarise(count = n())
